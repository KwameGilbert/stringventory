...
const LowStockList = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await productService.getLowStockProducts({ limit: 50 });
    ...
    <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 h-full flex flex-col hover-lift">
      <div className="flex items-center justify-between mb-6">
        ...
        <Link 
          ...
          className="flex items-center gap-1 text-xs font-medium text-white bg-linear-to-r from-red-500 to-rose-600 px-3 py-1.5 rounded-full hover:shadow-lg hover:shadow-red-200 transition-all group"
        >
          {items.length} items
      ...
      <div className="flex-1 space-y-3 overflow-y-auto max-h-75 pr-2 custom-scrollbar">
        {items.map((item, index) => {
          ...
          return (
            <div 
              key={index} 
              className="group p-4 rounded-xl border border-red-100 bg-linear-to-r from-red-50/50 to-rose-50/30 hover:from-red-50 hover:to-rose-50 transition-all cursor-pointer"
            >
              ...
              <div className="h-1.5 bg-gray-200 rounded-full overflow-hidden">
                <div 
                  className={`h-full bg-linear-to-r ${stock.color} rounded-full transition-all duration-500`}
