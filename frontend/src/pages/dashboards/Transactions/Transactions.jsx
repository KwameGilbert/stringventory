...
export default function Transactions() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (payload?.transactions) {
        const currency =
          response?.currency || response?.data?.currency || "GHS";
  ...
  const filteredTransactions = transactions.filter((tx) => {
    ...
    const matchesSearch =
      ...
      (tx.paymentMethod &&
        tx.paymentMethod.toLowerCase().includes(searchString));
    ...
    try {
      await exportToPDF({
        ...
        totals: [
          {
            label: "Total Inflow",
            value: formatPrice(Math.abs(summary.totalInflow), responseCurrency),
            color: "emerald",
          },
          {
            label: "Total Outflow",
            value: formatPrice(Math.abs(summary.totalOutflow), responseCurrency),
            color: "rose",
          },
          {
            label: "Net Balance",
            value: formatPrice(Math.abs(summary.netProfitLoss), responseCurrency),
            bold: true,
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm tracking-tight">
            Real-time ledger of all company cash flow
          </p>
        </div>
