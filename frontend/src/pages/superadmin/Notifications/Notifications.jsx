...
const normalizeNotification = (item) => {
  const type = (item?.type || 'default').toLowerCase();
...
export default function Notifications() {
  ...
  const loadNotifications = async () => {
    try {
      ...
      const list = Array.isArray(summary?.notifications)
  ...
  const tabs = useMemo(
    () => [
      ...
      { id: 'payment', label: 'Payments', count: notifications.filter((n) => n.type === 'payment').length },
      {
        id: 'system',
        label: 'System',
        count: notifications.filter((n) => n.type === 'system' || n.type === 'alert').length,
      },
    ],
    [notifications]
  );
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 px-4">
            ...
            <p className="text-sm text-gray-500">
              {activeTab === 'all' ? "You're all caught up! Check back later." : `No ${activeTab} notifications to show.`}
            </p>
          </div>
        ) : (
          <div className="divide-y divide-gray-100">
            {filteredNotifications.map((notification) => {
              ...
              return (
                ...
                >
                  <div
                    className={`w-10 h-10 rounded-lg flex items-center justify-center shrink-0 ${notification.bgColor}`}
                  >
                  ...
                  {!notification.read && <div className="w-2 h-2 bg-emerald-600 rounded-full shrink-0 mt-2" />}
                </div>
              );
            })}
          </div>
